...
color ambientColor(const color &mat, const color &light) {
	return glm::clamp(mat * light, 0.0f, 1.1f);
}
...
color totalColor(const Material &mat, const LightColor &lightColor,
	...
	glm::vec3 r = 2 * glm::dot(l, normal) * normal - l;

	color ambient = ambientColor(mat.ambient, lightColor.ambient);
	color diffuse = diffuseColor(mat.diffuse, lightColor.diffuse, lightPos, normal);
	color specular = specularColor(mat.specular, lightColor.specular, mat.shininess, r, viewingDir);
...
color PositionalLight::illuminate(const HitRecord &hit, const glm::vec3 &viewingDir,
									const Frame &eyeFrame, bool inShadow) const {
	if (!inShadow) {
		return hit.material.ambient + hit.material.diffuse + hit.material.specular;
	}
