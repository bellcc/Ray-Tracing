...
HitRecord VisibleIShape::findIntersection(const Ray &ray,
	const std::vector<VisibleIShapePtr> &surfaces) {
	HitRecord theHit;
	theHit.t = FLT_MAX;
...
void IDisk::findClosestIntersection(const Ray &ray, HitRecord &hit) const {
	...
	if (hit.t < FLT_MAX && glm::distance(hit.interceptPoint, center) <= radius) {
		// go to hit within disk
	}
	else {
		hit.t = FLT_MAX;
	}
